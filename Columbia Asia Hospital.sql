...
rename TABLE `doctor_patients_data (1)` TO doctor_patients_data;
rename TABLE `hospital er (1)` TO hospital_ER;
...
ORDER BY no_of_patients ASC,Revenue DESC
limit 5;
...
Comparison AS (
    ...
    LEFT JOIN Monthly_Avg_WaitingTime as prev1
        ...
        AND (curr.year = prev1.year AND curr.month = prev1.month + 1
        OR curr.year = prev1.year + 1 AND curr.month = 1 AND prev1.month = 12)
...
select
    ...
    round(sum(case when patient_gender = "M" then d.`Total Bill` end)
